...
/**
 * A PlayList comprises a list of <code>AudioSource</code>s, built from strings
 * that name file with an MP3 audio stream. Once the list is created, the
 * <code>AudioSources</audio> in the files can be played.
 */
...
public class PlayList implements Observer {
     ...
     * files. The list itself contains <code>AudioSource</code> objects created
     * from the filenames.
     *
    ...
    /**
     * Return the size of the play list (the number of <code>AudioSource</code>s
     * in it).
     *
    ...
    /**
     * Play a specific <code>AudioSource</code> in the list. When done playing,
     * the try to move on to the next <code>AudioSource</code> in the play list.
     *
     ...
     *
     * @return millisecond position in current stream or 0 if no stream is being
     * played
     */
    ...
    /**
     * Pause playback. Only has effect if the AudioPlayer is playing.
     */
    public void pause() {
        player.pause();
    }
    ...
    /**
     * Resume playback. Only has an effect if the AudioPlayer is paused.
     */
    public void resume() {
        player.resume();
    }
    ...
    /**
     * Update called when an audio stream finishes playing. This is only called
     * when the stream being played completes without interruption; the
     * assumption is that the client will decide to stop or will select a new
     * file to play.
     *
     * @param o The observable object sending the update
     * @param junk Ignored secondary information.
     */
    String nextSong = "next";
    ...
    /**
     * The internal method that initiates playback of the AudioSource selected
     * by <code>index</code>.
     *
     * Synchcronized because this is the only method that can be called from
     * both the main thread <i>and </i>the audio player thread.
     *
